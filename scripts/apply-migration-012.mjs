#!/usr/bin/env node
/**
 * Apply db/migrations/012_restore_workflow_add_link_options.sql using DATABASE_URL from .env.local.
 *
 * Usage (from project root):
 *   npm run db:apply-012
 */
import fs from "fs";
...
const sqlPath = path.join(
  ...
  "db",
  "migrations",
  "012_restore_workflow_add_link_options.sql",
);
...
try {
  ...
  console.log("Applied 012_restore_workflow_add_link_options.sql successfully.");
} finally {
  await client.end();
}
