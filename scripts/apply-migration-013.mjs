#!/usr/bin/env node
/**
 * Apply db/migrations/013_remap_step_links_to_link_options.sql using DATABASE_URL from .env.local.
 *
 * Usage (from project root):
 *   npm run db:apply-013
 */
import fs from "fs";
...
const sqlPath = path.join(
  ...
  "db",
  "migrations",
  "013_remap_step_links_to_link_options.sql",
);
...
try {
  ...
  console.log("Applied 013_remap_step_links_to_link_options.sql successfully.");
} finally {
  await client.end();
}
