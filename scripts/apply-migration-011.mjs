#!/usr/bin/env node
/**
 * Apply db/migrations/011_workflow_step_link_names.sql using DATABASE_URL from .env.local.
 *
 * Usage (from project root):
 *   npm run db:apply-011
 */
import fs from "fs";
...
const sqlPath = path.join(__dirname, "..", "db", "migrations", "011_workflow_step_link_names.sql");
...
try {
  ...
  console.log("Applied 011_workflow_step_link_names.sql successfully.");
} finally {
  await client.end();
}
