#!/usr/bin/env node
/**
 * Apply db/migrations/010_admin_change_log.sql using DATABASE_URL from .env.local.
 *
 * Usage (from project root):
 *   npm run db:apply-010
 */
import fs from "fs";
...
const sqlPath = path.join(__dirname, "..", "db", "migrations", "010_admin_change_log.sql");
...
try {
  ...
  console.log("Applied 010_admin_change_log.sql successfully.");
} finally {
  await client.end();
}
